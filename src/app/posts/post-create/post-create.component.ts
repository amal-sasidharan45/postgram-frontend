...
import { emptyPosts, Posts } from '../model/post.interface';
...
closeDropdown(index: number) {
  ...
  constructor(private modalService:BsModalService,private userService:AuthService,private postService:PostService){}
  ngOnInit(): void {
    this.show=true
   ...
   this.postService.getPostsForprofile().pipe(
    catchError((error)=>{
      return throwError(error)
    })
   ).subscribe({
...
onSaveProfile(){
  ...
  this.userService.UpdateUserProfile(this.form.value.name,this.form.value.username,this.form.value.description,this.form.value.image)
    .pipe(
      catchError((error) => {
        return throwError(error);
      })
...
confirmAction() {
  this.postService.deletePost(this.postId).pipe(catchError((error)=>{
    return throwError(error);
  })).subscribe({
    next:(data:any)=>{
...
onUpdate(){
  ...
  this.postService.updatePost(this.postId,Values.caption,Values.image,Values.creator,Values.userName,Values.profilePicture,Values.TimeDayWeek,Values.likes)
  .pipe(
    catchError((error)=>{
      return throwError(error);
    })
