...
constructor(private Router:Router,private ModalService:BsModalService ,private Authservice:AuthService,private postService:PostService,private sanitizer: DomSanitizer ){}
isHeartRed = false;
...
getPostsSearch(){
  ...
  this.postSub=this.postService.getPostUpdatedListener().subscribe((PostData:{posts:Posts[]})=>{
    ...
    if(this.userName){
      ...
      this.posts=PostData.posts.filter((name:any)=>
         ...
         UserNameWords.some((word)=>name.userName.toLowerCase().includes(word))
    )
    console.log(this.posts);
    this.likedPosts = this.posts.map(post => 
      post.likes.some(like => like.creator === this.userId && like.isLiked)
    );
...
return
  ...
  if (likeIndex !== -1) {
    ...
    console.log(post.likes[likeIndex].isLiked ? 'Increment like' : 'Decrement like');

    // Call the API to update the like status
    this.postService.updateLike(post.id, post.likes[likeIndex].isLiked, userId)
      .pipe(
        catchError((error) => {
    ...
    this.postService.updateLike(post.id, true, userId)
      .pipe(
        catchError((error) => {
          return throwError(error);
        })
...
onSavePost(){
  ...
  this.postService.addposts('Posts' ,this.form.value.caption,this.form.value.image,[]).subscribe((data:any)=>{
    console.log(data.post._doc);
  this.posts.push({caption:data.post._doc.caption,creator:data.post._doc.creator,imagePath:data.post._doc.imagePath,id:data.post._doc._id,userName:data.post._doc.userName,profilePicture:data.post._doc.profilePicture,TimeDayWeek:data.post._doc.TimeDayWeek,likes:data.post._doc.likes})
  console.log(this.posts);
...
onsaveComment(posId:string){
  ...
  this.postService.saveComment(posId,this.CommentForm.value.comment).pipe(
    catchError(err=>{
    ...
    next:(data:any)=>{
      ...
      this.comments.push({comment:data.comment.comment,postId:data.comment.postId,creator:data.comment.creator,creatorImg:data.comment.creatorImg,username:data.comment.username,duration:data.comment.duration,id:data.comment._id})
...
confirmAction(){
  ...
  this.postService.deleteComment(this.CommentId).pipe(
    catchError((err)=>{
      return throwError (err)
    })
  ).subscribe({
